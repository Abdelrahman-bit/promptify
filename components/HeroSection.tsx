...
const Orb = dynamic(() => import("@/components/ui/Orb"), {
	ssr: false,
	loading: () => <div className='w-full h-full bg-gradient-to-br from-[var(--magic-dark)] to-[var(--background)]' />,
});
...
const TextType = dynamic(() => import("@/components/ui/TextType"), {
	...
	loading: () => (
		<span
			className='bg-gradient-to-r from-[var(--magic-glow)] via-[var(--accent)] to-[var(--magic-glow-secondary)] 
...
export function HeroSection({ className }: HeroSectionProps) {
	...
	return (
		<section className={cn("relative w-full overflow-hidden flex-1", "flex items-center justify-center", className)}>
			{/* Orb Background */}
				...
				<div className='flex flex-col items-center text-center space-y-6 md:space-y-8'>
					...
					<div className='space-y-3'>
						...
						<p className='text-sm sm:text-base md:text-lg text-muted-foreground max-w-xl mx-auto leading-relaxed'>
							Enter your idea and let our AI enhance it into a clear, detailed prompt that any AI can understand and
							build.
						</p>
					</div>

					{/* Side-by-side layout on large screens, column on small */}
					<div className={cn("w-full", showOutput ? "grid grid-cols-1 lg:grid-cols-2 gap-6" : "max-w-2xl")}>
						{/* Input Card */}
							...
							<div className='relative rounded-2xl bg-[var(--card)]/80 backdrop-blur-sm border border-[var(--border)] p-4 md:p-5 space-y-4 h-full'>
                             ...
                             blur-xl transition-all duration-500 -z-10'
								...
								<div className='flex items-center justify-between gap-3 pt-2 border-t border-[var(--border)]/50'>
									...
									<ProductTypeSelector value={productType} onChange={setProductType} disabled={isEnhancing} />

									{/* Right side - Enhance Button */}
									<EnhanceButton onClick={handleEnhance} disabled={isEnhanceDisabled} isLoading={isEnhancing}>
						...
						{showOutput && (
							<div className='w-full min-h-[300px] lg:min-h-0'>
								<EnhancedOutput content={enhancedPrompt} isStreaming={isEnhancing} className='h-full' />
							</div>
						)}
					</div>
